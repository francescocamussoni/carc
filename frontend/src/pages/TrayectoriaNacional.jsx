...
function TrayectoriaNacional() {
  ...
  const fetchGame = async () => {
    try {
      setLoading(true)
      const data = await gamesAPI.getTrayectoriaNacional()
  ...
  const revelarClub = () => {
    if (clubesRevelados.length < gameData.clubes_nacionales.length && vidas > 0) {
      setclubesRevelados([...clubesRevelados, clubesRevelados.length])
  ...
  return (
    <div className="trayectoria-game rosario-central">
      <div className="game-header">
        <h1>🇦🇷 Trayectoria Nacional del Día</h1>
        <p className="game-subtitle">Adivina el jugador por su trayectoria en clubes argentinos</p>
      ...
      <div className="trayectoria-container">
        <h3>Trayectoria de Clubes</h3>
        <div className="clubes-grid">
          {gameData?.clubes_nacionales.map((club, index) => (
            <div
              key={index}
        ...
        <div className="game-controls">
          ...
          <div className="action-buttons">
            <button
              ...
              onClick={revelarClub}
              disabled={clubesRevelados.length >= gameData.clubes_nacionales.length || vidas <= 0}
            >
              Revelar Club (-1 vida)
            </button>
...
export default TrayectoriaNacional

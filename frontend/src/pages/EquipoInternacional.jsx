...
function EquipoInternacional() {
  ...
  const fetchGame = async () => {
    try {
      setLoading(true)
      const data = await gamesAPI.getEquipoInternacional()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.correcto) {
        ...
        setMensaje(`✅ ¡Correcto! ${jugadorRevelado.nombre} (${jugadorRevelado.nacionalidad}) - ${jugadorRevelado.posicion}`)
        
        if (jugadoresRevelados.length + 1 >= 11) {
          setGameOver(true)
          setMensaje('🎉 ¡Felicitaciones! Completaste el equipo internacional')
        }
      } else {
  ...
  const usarPista = () => {
    if (pistasUsadas < gameData.pistas_disponibles && !gameOver) {
      ...
      if (jugadoresNoRevelados.length > 0) {
        ...
        setMensaje(`💡 Pista: ${jugador.posicion} - ${jugador.nacionalidad}`)
  ...
  return (
    <div className="equipo-game internacional">
      <div className="game-header">
        <h1>🌎 Equipo Internacional del Día</h1>
...
export default EquipoInternacional
